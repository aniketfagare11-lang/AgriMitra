...
const CropScan = () => {
  ...
  const statusText = useMemo(() => {
    if (!fileName) return t('cropScan.uploadHint');
    return t('cropScan.selectedFile', { fileName });
  }, [fileName, t]);
    ...
    <div className="bg-background text-on-background min-h-screen font-body-md antialiased pb-28">
      ...
      <main className="w-full max-w-md mx-auto px-container-margin pt-6 flex flex-col gap-6">
        <section className="bg-[#064E3B]/60 backdrop-blur-[20px] rounded-xl p-card-padding border border-[rgba(240,253,250,0.1)] flex flex-col gap-4">
          <h1 className="font-h2 text-h2 text-on-surface">{t('cropScan.analyzer')}</h1>
          <p className="text-body-md text-on-surface-variant">{t('cropScan.subtitle')}</p>
          ...
          {previewUrl && (
            <img src={previewUrl} alt="Crop preview" className="w-full h-48 object-cover rounded-lg border border-outline-variant/40" />
          )}
          <button
            onClick={analyzeCrop}
            className="w-full bg-[#84CC16] text-[#022C22] py-3 rounded-lg font-label-sm hover:opacity-90 transition-opacity"
          >
            {loading ? 'Analyzing...' : t('cropScan.analyze')}
          </button>
